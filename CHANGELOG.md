...
- **Роль `common` ограничивает журнал systemd** — `common_journal_max_use`, по умолчанию `500M`,
  drop-in `/etc/systemd/journald.conf.d/krot-journal.conf`. Без лимита journald берёт 10%
  раздела, но не больше 4 ГБ (на диске busel в 77 ГБ упирается в потолок: 10% там были бы
  7.6 ГБ), и держит всё: журнал дорос до 1 ГБ, храня каждое сообщение с установки машины
  полутора месяцами ранее.

  Ограничен размер, а не срок хранения: потолок держит цену журнала при любой болтливости
  машины, тогда как срок ограничивает возраст и позволяет одному плохому дню занять диск.

  **Уже разросшийся журнал роль не ужимает.** Рестарт journald применяет лимит к новым записям,
  накопленное уходит при следующей ротации. Урезать сразу —
  `journalctl --rotate --vacuum-size=500M`: без `--rotate` vacuum убирает только архивные
  файлы и не трогает тот, в который журнал пишется сейчас. Роль этого не делает, потому что
  удаление истории там, где место ещё есть, — решение оператора, а не побочный эффект прогона.
