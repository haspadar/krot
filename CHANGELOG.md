...
- Роль `goaccess`: отчёты посещаемости по логам nginx, по одному на сайт, на loopback-порту за
  basic auth. Ни строки JS на сайтах — счётчик третьей стороны на сети доменов был бы уликой
  связи между ними. Общего отчёта нет намеренно: одна страница со всеми доменами и есть тот
  список, ради которого сеть прячут.
