...
- **Роль `cron` — периодические задачи приложения как systemd-таймеры.** Задача объявляется в
  ...
  Минуту выбирать не нужно, задачи разводит `RandomizedDelaySec`. `APP_ENV` и прочие переменные
  объявляются явно: юнит, как и cron, стартует с пустым окружением.
...
- **Роль `nginx` больше не ставит `/etc/logrotate.d/krot-nginx`.** Её glob совпадал с пакетным
  ...
  Переменная и её смысл не изменились.
