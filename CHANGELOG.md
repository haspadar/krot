...
- **Роль `common` ограничивает журнал systemd** — `common_journal_max_use`, по умолчанию `500M`,
  ...
  раздела (7.7 ГБ на диске 77 ГБ) и держит всё: на busel журнал дорос до 1 ГБ, храня каждое
  сообщение с установки машины полутора месяцами ранее.
  ...
  **Уже разросшийся журнал роль не ужимает.** Рестарт journald применяет лимит к новым записям,
  накопленное уходит при следующей ротации. Урезать сразу — `journalctl --vacuum-size=500M`;
  роль этого не делает, потому что удаление истории там, где место ещё есть, — решение
