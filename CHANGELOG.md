...
- **Человеческий отчёт требует и имени, и поведения.** Списка имён мало: часть сканеров ходит
  под видом обычного Chrome. Теперь адрес попадает в отчёт, только если сделал не меньше
  `goaccess_humans_min_referers` (по умолчанию 2) запросов с внутренним referer — так браузер,
  грузящий картинки отрисованной страницы, отличается от сканера, идущего по списку URL.
  Считается двумя проходами `awk` до GoAccess: он читает строки по одной и не помнит адрес.
- Порог 2, а не 1, — измерено: сканер `/.git/HEAD` сделал 53 запроса, из них **один** с
  referer, и проверку «есть хотя бы один» он проходил. На berlindame.de остаётся 6 адресов из
  103; при пороге 3 — 5, то есть ответ не балансирует на границе.
- Признаки нужны оба: **UptimeRobot сам себе ставит referer** и ловится только именем, а
  сканер под видом Chrome — только поведением.
- Разбор проверяет формат строки: адрес третьим полем только там, где впереди `[$time_local]`.
  В строках старого формата третье поле — имя хоста, и счёт шёл бы по нему.
