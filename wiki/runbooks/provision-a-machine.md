...
⚠️ **Проверять наличие копий надо отдельно, и есть чем.** Роль `backup` ставит `krot-restore` —
им же и проверяется, что копии есть и разворачиваются: он отказывает внятно, если набора нет,
если хранилище не отвечает или если копия пуста. Порядок и что кусается —
[копии баз](../operations/database-backups.md).
