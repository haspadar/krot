...
самый список, ради которого сеть прячут. По той же причине `autoindex off` — перечень файлов
выдал бы домены даже без чтения отчётов.
...
Отдаётся на `127.0.0.1:8443` за basic auth, то есть только через туннель:

```bash
ssh -L 8443:127.0.0.1:8443 <host>
# затем http://127.0.0.1:8443/<домен>.html
```

Поддомен потребовал бы CF-зоны, DNS-записи и сертификата, а трафик сети оказался бы в интернете
за одним паролем. При привязке к loopback файрволу нечего закрывать: снаружи порта просто нет.
