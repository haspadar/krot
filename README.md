...
`WorkingDirectory=`; `AssertPathIsDirectory=` для этого не годится — провалившийся assert
**не роняет unit**, а пропускает запуск, оставляя `Result=success` и пустой `--failed`
