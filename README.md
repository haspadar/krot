...
История хранится в собственной базе (`--persist`), поэтому переживает ротацию логов. Ротированные
`.gz` намеренно не перечитываются — они уже посчитаны, и повторный проход удвоил бы каждый хит.
