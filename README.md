...
### Один отчёт на сайт: весь трафик

На сайт строится **одна страница** — `<домен>.html`, весь трафик, включая ботов. Она отвечает
на вопрос, ради которого роль и заведена: обходят ли сайт, кто именно и какие страницы.

Второй отчёт — «только люди», `humans/<домен>.html` — в роли остался, но **выключен по
умолчанию** (`goaccess_humans_report`). Не потому, что не работал: потому, что его цифре
нельзя было верить.

Три захода на berlindame.de, каждый фильтр по-своему разумный:
...
И даже из этих шести трое грузили `/media/` с referer'ами ровно как браузер — по логу от людей
не отличаются в принципе. Честный ответ был **два**: владелец через VPN и его брат.

Число, которое под тремя одинаково обоснованными фильтрами уходит с 41 на 6, — не измерение.
Читается оно при этом как измерение, и в этом вред: отсутствующая цифра честнее выдуманной.
Отличать людей от машин по одному лишь рисунку запросов можно до предела, и предел этот
достигнут. Дальше нужна метка, которую исполняет браузер, — лог её дать не может.

Полного отчёта для задачи «смотреть, что происходит на сайте» достаточно.

Включение возвращает вторую страницу: роль строит её в том же прогоне, не дожидаясь таймера.
Выключение **удаляет** каталог `humans/` и `/etc/goaccess/krot-crawlers.list` — иначе страницы
остались бы висеть, замерев на дне выключения, рядом с полным отчётом, который обновляется.

Что накоплено на случай, если отчёт понадобится, — оно проверено на живом трафике и заново
добывалось бы с нуля:

- **Половина сканеров ботами себя не называет.** `--ignore-crawlers` ловит только тех, у кого
  в User-Agent есть `bot`, `crawler` или `spider`, а `GoogleOther`, `Dataprovider`,
  `InternetMeasurement` и HTTP-библиотеки (`curl`, `Go-http-client`, `okhttp`) проходят как
  посетители. Отсюда `goaccess_extra_crawlers` — файл `/etc/goaccess/krot-crawlers.list`,
  подключаемый флагом `-b` только ко второму отчёту.
...
- **`UptimeRobot` ставит себе внутренний referer** на `HEAD /` — поведенческая проверка его
  пропускает, ловит только имя. И наоборот, сканер под видом Chrome невидим для списка имён.
  Ни один признак поодиночке не работает, и это было главным доводом их сложить.
- `--unknowns-as-crawlers` не используется: на замере он сдвинул счёт на один визит из 37.
