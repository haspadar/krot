...
def write_index(path: Path, lines: list[str]) -> None:
    ...
    любого коммита — `git diff` видит новую дату при том же содержимом, и проверка
    «индекс пересобран» превращается в календарный будильник.
    """
    new_contents = "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    lines = [
        ...
        "",
        "Номер PR восстановлен из merge-коммита по совпадению имени ветки с именем change "
        "после отбрасывания даты и префикса типа (`feat/`, `fix/`). Сравнение остаётся "
        f"строгим, поэтому не восстановлен у **{unresolved}** из {len(all_changes)}: "
        "нестрогое приписало бы change чужой PR — прочерк честнее неверного номера.",
        "",
