...
- [x] Вики: страница про запуск сайта (порядок, переменные, что подключает проект)
- [x] Вики: `collection-layout.md` — два набора ролей (машина / сайт), раздел про Terraform
- [x] `openspec/ARCHITECTURE.md`, `README.md` — тот же пересмотр
- [x] `galaxy.yml`: минорная версия, описание; CHANGELOG 5.5.0 (и пропущенная 5.4.0)
- [x] Двойное ревью до push: Codex — имя базы без владельца падало уже после записей (проверка
  перенесена в preflight), «половины сертификата переезжают вместе» неточно (текст исправлен);
  свой агент в роли разработчика busel — расхождений с кодом нет, ссылка на архив верна после
  архивации, gudok в ARCHITECTURE назван пользователем ролей раньше перехода (исправлено)
- [x] Архивация change последним коммитом

## Приёмка

- [x] `--check` на `tbl-telefon.de` (2026-09-26, временный инвентарь вне репозиториев, значения
  от сессии gudok): выход 0, `changed=2` — оба раза одна настройка зоны. Зона active,
  делегирование, пара сертификата, сайт в Umami, подтверждения Google, Яндекса и Bing, записи,
  sitemap и смоук снаружи — уже на месте, ничего не создавалось бы.
- [ ] Живой прогон на `tbl-telefon.de` — **не выполнялся**: он изменил бы `browser_cache_ttl`
  зоны с 14400 (умолчание Cloudflare, 4 ч, заменяет Cache-Control сайта) на 0. Это ровно грабля
  из плана, но это правка живого сайта — решение за владельцем. Переход gudok на роли вне change.
