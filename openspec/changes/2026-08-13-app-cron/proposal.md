...
- **Чинится `logrotate.service`.** Роль `nginx` перестаёт дублировать пакетный glob: `krot-nginx`
  снимается, параметры ротации nginx едут в drop-in штатного файла или файл не ставится вовсе
  (решение — в tasks.md). Пока этого нет, на машине **не ротируется ничего**, и это надо
  понимать, читая сегодняшний `/var/log`.
