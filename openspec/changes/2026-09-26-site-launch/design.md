...
- **`site_search`**: sitemap сперва запрашивается без авторизации и должен ответить 200 **телом
  sitemap** (`<urlset>` или `<sitemapindex>`): маршрут-ловушка отвечает 200 главной на любой
  путь. Путь — переменная (у gudok скрытый `/sitemap-<токен>.xml`).
- **`site_tls` до `site_dns_records`** (ревью PR 3): настройки зоны включают strict, и на живой
  зоне с сертификатом не от Origin CA и не публичным это ломает сайт. Какие настройки изменятся,
  `site_preflight` называет в отчёте.
- **Одна машина на прогон**: preflight требует ровно один хост в play, до первой записи.
  Облачные шаги идут `run_once` с переменными первого хоста, и второй получил бы машину под чужую
  зону.
- **A-запись с тем же адресом, но без прокси**, переводится под прокси — это замысел, а не
  побочный эффект; `--check` показывает её как changed.
...
Найдено ревью Codex PR 3. Обещание «`--check` ничего не пишет» держат две вещи, а не
добросовестность проекта:

- плейбук под `--check` не вызывает пишущие шаги проекта (fill, results_writer, deploy, open);
- у сайта без зоны пробный прогон кончается отчётом `site_preflight`: дальше каждый шаг читает
  зону, и без неё прогон упал бы на первом же пустом значении, а не на плане.

Сценарий molecule начинает с шага `check` на пустом состоянии; подделка складывает каждый
пишущий запрос под прогон, который его прислал, и `verify.yml` требует у пробного — ни одного.
Повторный прогон может слать только sitemap: его отправка повторяется на каждом прогоне
намеренно.

Подделка сайта отвечает так, как увидит посетитель: 530 без proxied-записи в активной зоне,
401 до шага открытия, 200 — после. Поисковики подтверждают домен, только найдя свою запись в
активной зоне. Иначе сценарий был бы зелёным и при запуске, который ничего не опубликовал.
