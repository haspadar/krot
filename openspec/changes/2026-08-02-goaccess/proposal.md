...
### Persist вместо чтения ротированных файлов

`--persist/--restore` с базой на диск: история переживает файлы, из которых получена. Иначе
каждая ротация обнуляла бы статистику до того, что осталось на диске. Ротированные `.gz`
намеренно **не** скармливаются — они уже посчитаны в базу, и повторный проход удвоил бы каждый
хит.
